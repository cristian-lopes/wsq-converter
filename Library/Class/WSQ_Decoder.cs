...
namespace Fingerprint.Converter.Class
{
    public class WSQ_Decoder
    {
        public Bitmap Decode(byte[] data) {
            ...
            /* Read the SOI marker. */
            getCMarkerWSQ(token, WSQ_Helper.SOI_WSQ);

            /* Read in supporting tables up to the SOF marker. */
            int marker = getCMarkerWSQ(token, WSQ_Helper.TBLS_N_SOF);
            while (marker != WSQ_Helper.SOF_WSQ) {
                getCTableWSQ(token, marker);
                marker = getCMarkerWSQ(token, WSQ_Helper.TBLS_N_SOF);
            }
        ...
        private int getCMarkerWSQ(Token token, int type) {
            ...
            switch (type) {
                case WSQ_Helper.SOI_WSQ:
                    if (marker != WSQ_Helper.SOI_WSQ) {
                        throw new SystemException("ERROR : getCMarkerWSQ : No SOI marker : " + marker);
                    }

                    return marker;

                case WSQ_Helper.TBLS_N_SOF:
                    if (marker != WSQ_Helper.DTT_WSQ
                            && marker != WSQ_Helper.DQT_WSQ
                            && marker != WSQ_Helper.DHT_WSQ
                            && marker != WSQ_Helper.SOF_WSQ
                            && marker != WSQ_Helper.COM_WSQ) {
                        throw new SystemException("ERROR : getc_marker_wsq : No SOF, Table, or comment markers : " + marker);
                    }

                    return marker;

                case WSQ_Helper.TBLS_N_SOB:
                    if (marker != WSQ_Helper.DTT_WSQ
                            && marker != WSQ_Helper.DQT_WSQ
                            && marker != WSQ_Helper.DHT_WSQ
                            && marker != WSQ_Helper.SOB_WSQ
                            && marker != WSQ_Helper.COM_WSQ) {
                        throw new SystemException("ERROR : getc_marker_wsq : No SOB, Table, or comment markers : " +
                                marker);
                    }
                    return marker;
                case WSQ_Helper.ANY_WSQ:
                    if ((marker & 0xff00) != 0xff00) {
                        throw new SystemException("ERROR : getc_marker_wsq : no marker found : " + marker);
                    }

                    /* Added by MDG on 03-07-05 */
                    if ((marker < WSQ_Helper.SOI_WSQ) || (marker > WSQ_Helper.COM_WSQ)) {
        ...
        private void getCTableWSQ(Token token, int marker) {
            switch (marker) {
                case WSQ_Helper.DTT_WSQ:
                    getCTransformTable(token);
                    return;
                case WSQ_Helper.DQT_WSQ:
                    getCQuantizationTable(token);
                    return;
                case WSQ_Helper.DHT_WSQ:
                    getCHuffmanTableWSQ(token);
                    return;
                case WSQ_Helper.COM_WSQ:
        ...
        public void getCHuffmanTableWSQ(Token token) {
            ...
            HuffmanTable firstHuffmanTable = getCHuffmanTable(token, WSQ_Helper.MAX_HUFFCOUNTS_WSQ, 0, true);
            ...
            while (bytesLeft != 0) {
                ...
                HuffmanTable huffmantable = getCHuffmanTable(token, WSQ_Helper.MAX_HUFFCOUNTS_WSQ, bytesLeft, false);
        ...
        private HuffmanTable getCHuffmanTable(Token token, int maxHuffcounts, int bytesLeft, bool readTableLen) {
            ...
            huffmanTable.huffbits = new int[WSQ_Helper.MAX_HUFFBITS];
            int numHufvals = 0;
            /* L1 ... L16 */
            for (int i = 0; i < WSQ_Helper.MAX_HUFFBITS; i++) {
                huffmanTable.huffbits[i] = token.readByte();
                numHufvals += huffmanTable.huffbits[i];
            }
            huffmanTable.bytesLeft -= WSQ_Helper.MAX_HUFFBITS;
        ...
        private void buildWSQTrees(Token token, int width, int height) {
            ...
            buildWTree(token, WSQ_Helper.W_TREELEN, width, height);
            /* Build a Q-TREE structure for the image. */
            buildQTree(token, WSQ_Helper.Q_TREELEN);
        }
        ...
        private int[] huffmanDecodeDataMem(Token token, int size) {
            ...
            int[] maxcode = new int[WSQ_Helper.MAX_HUFFBITS + 1];
            int[] mincode = new int[WSQ_Helper.MAX_HUFFBITS + 1];
            int[] valptr = new int[WSQ_Helper.MAX_HUFFBITS + 1];

            IntRef marker = new IntRef(getCMarkerWSQ(token, WSQ_Helper.TBLS_N_SOB));
            ...
            while (marker.value != WSQ_Helper.EOI_WSQ) {

                if (marker.value != 0) {
                    while (marker.value != WSQ_Helper.SOB_WSQ) {
                        getCTableWSQ(token, marker.value);
                        marker.value = getCMarkerWSQ(token, WSQ_Helper.TBLS_N_SOB);
                    }
                    ...
                    HuffCode[] hufftable = buildHuffsizes(token.tableDHT[hufftableId].huffbits, WSQ_Helper.MAX_HUFFCOUNTS_WSQ);
        ...
        private HuffCode[] buildHuffsizes(int[] huffbits, int maxHuffcounts) {
            ...
            int tempSize = 0;
            for (int codeSize = 1; codeSize <= WSQ_Helper.MAX_HUFFBITS; codeSize++) {
        ...
        private void genDecodeTable(HuffCode[] huffcodeTable, int[] maxcode, int[] mincode, int[] valptr, int[] huffbits) {
            ...
            int i2 = 0;
            for (int i = 1; i <= WSQ_Helper.MAX_HUFFBITS; i++) {
                if (huffbits[i - 1] == 0) {
                    maxcode[i] = -1;
                    continue;
                }
        ...
        private int getCNextbitsWSQ(Token token, IntRef marker, IntRef bitCount, int bitsReq, IntRef nextByte) {
            ...
            if (bitsReq <= bitCount.value) {
                bits = (nextByte.value >> (bitCount.value - bitsReq)) & (WSQ_Helper.BITMASK[bitsReq]);
                bitCount.value -= bitsReq;
                nextByte.value &= WSQ_Helper.BITMASK[bitCount.value];
            } else {
        ...
        private float[] unquantize(Token token, int[] sip, int width, int height) {
            ...
            int sptr = 0;
            for (int cnt = 0; cnt < WSQ_Helper.NUM_SUBBANDS; cnt++) {
        ...
        private void wsqReconstruct(Token token, float[] fdata, int width, int height) {
            ...
            for (int node = WSQ_Helper.W_TREELEN - 1; node >= 0; node--) {
